---
applyTo: '**/Dockerfile,**/Dockerfile.*,**/*.dockerfile,**/docker-compose*.yml,**/docker-compose*.yaml,**/compose*.yml,**/compose*.yaml, ./docker-output/**/*.sh, ./docker-output/**/*.ps1'
...
- **Guidance for Agent:**
    - Recommend integrating tools like `hadolint` (for Dockerfile linting) and `Trivy`, `Clair`, or `Snyk Container` (for image vulnerability scanning) into your CI pipeline or built scripts.
...
```
- **Example (Security Scanning with a Bash script):**
```bash
#!/usr/bin/env bash
set -euo pipefail
...
echo "==> Done"
```
-- **Example (Security Scanning with a Powershell script):**
```powershell
...
Write-Host "==> Running Hadolint on $DockerfilePath"
Get-Content $DockerfilePath | docker run --rm -i hadolint/hadolint
if ($LASTEXITCODE -ne 0) { throw "Hadolint found issues (exit code $LASTEXITCODE)" }

Write-Host "==> Building image $ImageName"
docker build -t $ImageName .
if ($LASTEXITCODE -ne 0) { throw "Docker build failed (exit code $LASTEXITCODE)" }

Write-Host "==> Scanning $ImageName with Trivy"
trivy image --exit-code 1 --severity HIGH,CRITICAL $ImageName
if ($LASTEXITCODE -ne 0) { throw "Trivy found HIGH/CRITICAL vulnerabilities (exit code $LASTEXITCODE)" }
